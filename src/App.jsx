import './App.css'
import Navbar from './component/navbar'

function App() {

  return (
    <>
      <Navbar />
      <div>
        {/* container to write the English Word

        <div className='card'>
          <h3> Word : </h3>
          <h5>
            Explanation :
          </h5>
        </div> */}

        <center> <h1>Professional Communication</h1></center>
        <br />
        <h2> Introduction : </h2>

        {/* PROFESSIONAL COMMUNICATION SKILL */}
        <details>
          <summary className='ml'>
            Professional Communication Skills
          </summary>

          <details>
            <summary className='ml1'>
              English Language Skills
            </summary>

            <details>
              <summary className='ml2'>
                Grammer
              </summary>
              <details>
                <summary className='ml3'>Parts of speech</summary>


                <details>
                  <summary className='ml4'>
                    Verb
                  </summary>
                  <div className="box">
                    <p>
                      a word or group of words that is used to indicate that something happens or exists, for example bring, happen, be, do
                    </p>
                  </div>
                </details>

                <details>
                  <summary className='ml4'>
                    Noun
                  </summary>
                </details>


                <details>
                  <summary className='ml4'>
                    Pronoun
                  </summary>
                </details>


                <details>
                  <summary className='ml4'>
                    Adjective
                  </summary>
                </details>

                <details>
                  <summary className='ml4'>
                    Adverb
                  </summary>
                </details>

                <details>
                  <summary className='ml4'>
                    Preposition
                  </summary>
                </details>

                <details>
                  <summary className='ml4'>
                    Conjuction
                  </summary>
                </details>

                <details>
                  <summary className='ml4'>
                    Interjection
                  </summary>
                </details>


              </details>
              <ul>
                <li>Phrase and clause.</li>
                <li>Kind of Sentence.</li>
                <li>Simple/Complex/Compound Sentence</li>
                <li>Infinitive and Gerund (Verb) </li>
                <li>Non finite</li>
                <li>Subject -Verb -Aggrement (Basic / Foundation of English Grammer)</li>
                <li>Verb and it's Usage.</li>
                <li>Use of Tenses</li>
                <li>Use of Voice</li>
                <li>Modals</li>
                <li>Determiners</li>
              </ul>

            </details>
            <details>
              <summary className='ml2'>
                Non Grammer
              </summary>

              <details>
                <summary>
                  Vocabulary Building
                </summary>

                <ul>
                  <li>The Concept of Word Formation.</li>
                  <li>Root words from Foreign languages and their use in English.</li>
                  <li>Acquaintance with prefixes and suffixes from Foreign languages.</li>
                  <li>In English to form Derivatives.</li>
                  <li>Synonyms, Antonyms, and Standard Abreviations.</li>
                  <li>Affixes, Acronyms.</li>
                </ul>
              </details>

            </details>
          </details>

          <details>
            <summary className='ml1'>
              Communication Skills
            </summary>

          </details>
        </details>

        {/* BASIC WRITING SKILLS */}
        <details>
          <summary className='ml'> Basic Writing Skills  </summary>


          <details>
            <summary className='ml1'>
              Sentence Structure
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Use of Phrases and Clauses in Sentences.
            </summary>
          </details>


          <details>
            <summary className='ml1'>
              Importance of proper Punctuation.
            </summary>
          </details>


          <details>
            <summary className='ml1'>
              Kinds of Sentences.
            </summary>
          </details>


          <details>
            <summary className='ml1'>
              Use of Tense, Use in Context and Coherence of Tense in writing.
            </summary>
          </details>


          <details>
            <summary className='ml1'>
              Use of Voice - Active/Passive in Sentence.
            </summary>
          </details>


          <details>
            <summary className='ml1'>
              Use of Speech - Direct and Indirect Speech.
            </summary>
          </details>


          <details>
            <summary className='ml1'>Framing Questions - Direct, Using modal.</summary>
          </details>

        </details>

        {/* IDENTIFYING COMMON ERRORS IN WRITING */}
        <details>
          <summary className='ml'>
            Identifying Common Errors in Writing
          </summary>

          <details>
            <summary className='ml1'>
              Subject-Verb Aggrement
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Noun-Pronoun Aggrement
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Misplaced Modifiers
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Articles
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Prepositions
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Redundancies
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Cliches
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Common English Errors
            </summary>
          </details>

        </details>

        {/* NATURE AND STYLE OF SENSIBLE WRITING */}
        <details>
          <summary className="ml">
            Nature and Style of Sensible Writing
          </summary>

          <details>
            <summary className='ml1'>
              Describing
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Defining
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Classifying
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Providing Examples and Evidence
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Writing Introduction and Conclusion
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Organizing Principle of paragraphs in Documents.
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Argument, Describing/Narrating/Planning, Defining, Classifying
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Lexical Resources, Using suitable language register.
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Coherance, Writing Introduction, Body and Conclusion, Techniques for writing precisely, Grammer and Accuracy.
            </summary>
          </details>

        </details>


        {/* WRITING PRACTICES AND COMPREHENSION */}
        <details>
          <summary className='ml'>
            Writing practices and Comprehension.
          </summary>

          <details>
            <summary className='ml1'>
              Formal letter writing / Application / Report Writing / Writing minutes of meetings.
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Essay Writing
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Formal Email Writing
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Resume / CV Writing / Cover letter.
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Statement of purpose.
            </summary>
          </details>

        </details>

        {/* ORAL COMMUNICATION */}
        <details>
          <summary className='ml'>
            Oral Communication
          </summary>

          <details>
            <summary className='ml1'>
              Listening Comprehension.
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Pronounciation, Intonation, Stress and Rhythm
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Common Everyday situations : Conversations and Dialogues.
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Communication at Workplace
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Interviews
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Formal Presentation.
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Acquanting Students with IPA symbols.
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Phonetic (Basic)
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Sounds - Vowels, Consonants
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Clearing mother tongue influence.
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Clearing Refundancies and Common Errors related to Indianisms group discussion.
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Expressing Opinions
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Coherence and Fluency in Speech.
            </summary>
          </details>

        </details>

        {/* PROFESSIONAL SKILLS */}
        <details>
          <summary className='ml'>
            Professional Skills
          </summary>

          <details>
            <summary className='ml1'>
              Team Building
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Soft Skills and Etiquettes
            </summary>
          </details>

        </details>

        {/* COMMUNICATION AND SOFT SKILLS */}
        <details>
          <summary className='ml'>
            Communication and Soft Skills
          </summary>


          <details>
            <summary className='ml1'>
              Interpersonal Communication Skill.
            </summary>

            <details>
              <summary className='ml2'>
                Introduction to the course.
              </summary>
            </details>

            <details>
              <summary className='ml2'>
                What are Interpersonal Skills?
              </summary>
            </details>

            <details>
              <summary className='ml2'>
                Interpersonal skills to posses?
              </summary>
            </details>

            <details>
              <summary className='ml2'>
                How to develop interpersonal Skills.
              </summary>
            </details>

            <details>
              <summary className='ml2'>
                Importance of Interpersonal Skills.
              </summary>
            </details>

          </details>

          <details>
            <summary className='ml1'>
              Effective Communication and Body Language.
            </summary>

            <details>
              <summary className='ml2'>
                What are Effective Communication?
              </summary>
            </details>

            <details>
              <summary className='ml2'>
                Importance of Effective Communication.
              </summary>
            </details>

            <details>
              <summary className='ml2'>
                What are different modes of Effective Communication.
              </summary>
            </details>

            <details>
              <summary className='ml2'>
                How to improve Body language.
              </summary>
            </details>

            <details>
              <summary className='ml2'>
                What are some Important Body gestures.
              </summary>
            </details>

            <details>
              <summary className='ml2'>
                What is meta Communications.
              </summary>
            </details>

          </details>

        </details>

        {/* OTHER IMPORTANT SOFT SKILLS */}
        <details>
          <summary className='ml'>
            Other Important Soft Skills.
          </summary>

          <details>
            <summary className='ml1'>
              What are personal skills?
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              What is decision making?
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              How to take good decision?
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Creative decision making?
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              What is assertiveness?
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              What is Team Work?
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              What are leadership skills
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              What is self motivation?
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Effective time management?
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              How to make Presentation?
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Role of dressing up and appearance.
            </summary>
          </details>

        </details>

        {/* ACTIVITIES */}
        <details>
          <summary className='ml'>
            Activities.
          </summary>

          <details>
            <summary className='ml1'>
              Narrative Chain
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Describing/Narrating
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Writing Essays in Relay
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Peer / Group Activities
            </summary>
          </details>

          <details>
            <summary className='ml1'>
              Brainstorming Vocabulary cue / Flash cards for Vocabulary.
            </summary>
          </details>

        </details>
      </div >

    </>
  )
}

export default App
